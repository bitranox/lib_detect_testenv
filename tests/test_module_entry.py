...
def test_when_module_entry_returns_zero_the_story_matches_cli(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(sys, "argv", ["lib_detect_testenv", "hello"], raising=False)

    with pytest.raises(SystemExit) as exc:
        runpy.run_module("lib_detect_testenv.__main__", run_name="__main__")
...
def test_when_module_entry_raises_the_exit_helpers_format_the_song(
    ...
    monkeypatch.setattr(sys, "argv", ["lib_detect_testenv", "fail"], raising=False)
...
def test_when_traceback_flag_is_used_via_module_entry_the_full_poem_is_printed(
    ...
    monkeypatch.setattr(sys, "argv", ["lib_detect_testenv", "--traceback", "fail"])
